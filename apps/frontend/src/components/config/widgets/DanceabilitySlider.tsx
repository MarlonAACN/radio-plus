...
function DanceabilitySliderWidget({
   ...
   */
  function toSliderValueFromConfigValue(value: number): number {
    return value * 100;
  }
   ...
   */
  function toConfigValueFromSliderValue(value: number): number {
    return value / 100;
  }
  ...
  function changeHandler(e: FormEvent<HTMLInputElement>) {
    ...
    updateInputChangeTracker({
      ...inputChangeTracker,
      danceability: toConfigValueFromSliderValue(Number(eventProgressValue)),
    });

    cachedDanceabilityValue.current = toConfigValueFromSliderValue(
      Number(eventProgressValue)
    );
  }

  return (
    <div className="radio-plus-danceability-slider-container w-full">
